...
export class Migration20230526225500 extends Migration {
	async up(): Promise<void> {
		this.addSql(
			'alter table "rental_asset" alter column "growth_rate" type int using ("growth_rate"::int);'
		);
		...
		this.addSql(
			'alter table "financial_asset" alter column "growth_rate" type int using ("growth_rate"::int);'
		);
		...
		this.addSql(
			'alter table "dividend" alter column "growth_rate" type int using ("growth_rate"::int);'
		);
		...
		this.addSql(
			'alter table "cap_asset" alter column "growth_rate" type int using ("growth_rate"::int);'
		);
		...
		this.addSql(
			'alter table "budget_item" alter column "growth_rate" type int using ("growth_rate"::int);'
		);
	...
	async down(): Promise<void> {
		...
		this.addSql(
			"alter table \"financial_asset\" add column \"fin_type\" text check (\"fin_type\" in ('daily', 'weekly', 'monthly', 'annually', 'non-reoccurring')) not null, add column \"brokerage_dividends\" int not null;"
		);

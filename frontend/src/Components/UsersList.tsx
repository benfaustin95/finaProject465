...
export const MacroReportSet = () => {
	...
	const loadReport = (event) => {
		if(endDate != "")
			navigate("/macroReportLoaded", {state:{end: endDate}});
		else
			setSubStatus(true);
	};
	...
	return (
		<div>
			<form className="grid grid-cols-4 gap-2 flex items-center text-right " onSubmit={loadReport}>
				<label htmlFor="endYear " className="col-span-1 text-blue-300 ">
					End Year:
				</label>
				<input type="date"
					   name="endDate"
				...
				/>
				<input className="btn btn-primary btn-circle ml-2 col-span-1 text-white border-4" type="submit" value="Load" id="send"/>
			</form>
		</div>
	);
};
